  ...
  <div class="batch-analysis-page">
    ...
    <div class="main-content">
      ...
      <div class="upload-section" v-if="batchStatus === 'idle' || batchStatus === 'uploading' || batchStatus === 'splitting'">
        ...
        <div class="input-section" v-if="batchStatus === 'idle'">
          ...
          <div class="input-tip">
            ...
            >
              <template #title>
                <span style="font-size: 13px;">前六个表的固定顺序为：<strong>最后操作分布</strong>、<strong>新手漏斗</strong>、<strong>回流用户</strong>、<strong>流失用户属性</strong>、<strong>留存率</strong>、<strong>LTV</strong></span>
              </template>
            </el-alert>
          </div>
      ...
      <div class="empty-state" v-if="batchStatus === 'idle' && !batchSessionId && fileList.length === 0 && batchReports.length === 0">
        <el-empty description="请上传包含多个Sheet的Excel文件开始定制化批量分析" />
      </div>
    </div>
...
const handleFileChange = async (file: UploadFile) => {
  ...
  try {
    ...
    const response = await uploadCustomBatchExcel(
      file.raw,
...
const startAnalysis = async () => {
  ...
  try {
    const response = await startCustomBatchAnalysis(
...
const saveWorkflowConfig = async () => {
  ...
  try {
    ...
    if (currentWorkflow.value) {
      ...
      // 绑定工作流到当前功能（用户级绑定，使用定制化批量分析的function_key）
...
// 加载工作流配置（优先使用定制化批量分析的function_key，如果没有则回退到普通工作流）
